...
CONFIG_PRIV_TIMER:                             
				...
				LDR		R1, =50000000				// load timer with 0.25s intervals
				STR		R1, [R0]
				
				MOV		R1, #0x7					// set interrupt mask, automatic and enable bits
...
CHECK_IDS:		
				MOV		 R6, #0					// Assume given IDs are NOT valid
				
				CMP      R5, #73				// pushbuttons
				MOVEQ	 R6, #1
				BLEQ	 KEY_ISR				// handle the pushbutton interrupt
				
				CMP		 R5, #29				// Priv timer
				MOVEQ	 R6, #1
				BLEQ	 PRIV_TIMER_ISR			// handle the timer interrupt
				
				CMP		 R6, #0
				BEQ		 UNEXPECTED				// If unexpected, go to unexpected loop
				
				B		 EXIT_IRQ				// Else exit the IRQ
...
KEY_ISR:        
				...
				MOV		 R1, #0xF				
				LDR		 R2, [R0, #0xC]			// get the key that was pressed
				STR		 R1, [R0, #0xC]			// reset all the interrupt bits
				
				LDR		 R3, =0xFFFEC600		// R3 gets location of timer data
				
				CMP		 R2, #1					// If KEY0 pressed
				LDREQ	 R1, RUN				// R1 gets RUN value
				EOREQ	 R1, #1					// RUN gets opposite value
				STREQ	 R1, RUN				// Put value back in RUN
				
				CMP		 R2, #2					// If KEY1 gets pressed
				LDREQ	 R1, [R3]				// Get current time factor
				LSREQ	 R1, #1					// Speed it up by 2
				STREQ	 R1, [R3]
				
				CMP		 R2, #4					// If KEY2 gets pressed
				LDREQ	 R1, [R3]				// Get current time factor
				LSLEQ	 R1, #1					// Slow it down by 2
				STREQ	 R1, [R3]
 ...
 *****************************************************************************/
                .global    TIMER_ISR
PRIV_TIMER_ISR:
                LDR		 R0, =0xFFFEC600		// base address of priv timer
				MOV		 R1, #1					// reset F interrupt bit
				STR		 R1, [R0, #0xC]	
				
				LDR		 R1, COUNT				// get COUNT global variable
				LDR		 R2, RUN				// get RUN global variable
				
				ADD		 R1, R2					// add run to count
				STR		 R1, COUNT				// store value back into COUNT
